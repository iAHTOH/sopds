...
def cmdtrans(func):
    def wrapper(self, bot, update):
        translation.activate(config.EOPDS_LANGUAGE)
...
def CheckAuthDecorator(func):
    def wrapper(self, bot, update):
        if not config.EOPDS_TELEBOT_AUTH:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.pidfile = os.path.join(main_settings.BASE_DIR, config.EOPDS_TELEBOT_PID)
        ...
        if settings.LOGLEVEL!=logging.NOTSET:
            ...
            fh = logging.FileHandler(config.EOPDS_TELEBOT_LOG)
    ...
    def BookPager(self, books, page_num, query):
        ...
        op = OPDS_Paginator(books_count, 0, page_num, config.EOPDS_TELEBOT_MAXITEMS, HALF_PAGES_LINKS)
        items = []
        ...
        summary_DOUBLES_HIDE = config.EOPDS_DOUBLES_HIDE
    ...
    def getBookFile(self, bot, update):
        ...
        if re.match(r'/getfileorig',query):
            ...
            #document = config.EOPDS_SITE_ROOT + reverse("opds_catalog:download",kwargs={"book_id": book.id, "zip_flag": 0})

        elif re.match(r'/getfilezip',query):
            document = dl.getFileDataZip(book)
            #document = config.EOPDS_SITE_ROOT + reverse("opds_catalog:download", kwargs={"book_id": book.id, "zip_flag": 1})
    ...
    def start(self):
        ...
        self.stdout.write("Quit the eopds_telebot with %s.\n"%quit_command)
        try:
            updater = Updater(token=config.EOPDS_TELEBOT_API_TOKEN)
    ...
    def stop(self, pid):
        try:
            ...
            self.stdout.write("Error stopping eopds_telebot: %s"%str(e))
...
def daemonize():
    ...
    std_out = open(config.EOPDS_TELEBOT_LOG, 'a+')
