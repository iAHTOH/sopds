...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options): 
        self.pidfile = os.path.join(main_settings.BASE_DIR, config.EOPDS_SCANNER_PID)
        ...
        if settings.LOGLEVEL!=logging.NOTSET:
            ...
            fh = logging.FileHandler(config.EOPDS_SCANNER_LOG)
    ...
    def update_shedule(self):
        self.SCAN_SHED_DAY = config.EOPDS_SCAN_SHED_DAY
        self.SCAN_SHED_DOW = config.EOPDS_SCAN_SHED_DOW
        self.SCAN_SHED_HOUR = config.EOPDS_SCAN_SHED_HOUR
        self.SCAN_SHED_MIN = config.EOPDS_SCAN_SHED_MIN
    ...
    def check_settings(self):
        ...
        if not (self.SCAN_SHED_MIN==config.EOPDS_SCAN_SHED_MIN and \
           self.SCAN_SHED_HOUR==config.EOPDS_SCAN_SHED_HOUR and \
           self.SCAN_SHED_DOW==config.EOPDS_SCAN_SHED_DOW and \
           self.SCAN_SHED_DAY==config.EOPDS_SCAN_SHED_DAY):
            self.update_shedule()
        if config.EOPDS_SCAN_START_DIRECTLY:
            config.EOPDS_SCAN_START_DIRECTLY = False
            self.stdout.write('Startup scannyng directly by eopds_SCAN_START_DIRECTLY flag.')
    ...
    def stop(self, pid):
        try:
            ...
            self.stdout.write("Error stopping eopds_scanner: %s"%str(e))
...
def daemonize():
    ...
    std_out = open(config.EOPDS_SCANNER_LOG, 'a+')
