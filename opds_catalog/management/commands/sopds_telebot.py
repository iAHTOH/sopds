...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--daemon', action='store_true', dest='daemonize', default=False,
    ...
    async def _with_lang(self, func, update, context):
    ...
    async def _with_auth(self, func, update, context):
        ...
        username = (update.message.from_user.username if update.message
        ...
        await context.bot.send_message(
            ...
            text=_("Hello %s!\nUnfortunately you do not have access to information. "
                   "Please contact the bot administrator.") % username
        )
    ...
    async def _startCommand(self, update, context):
        return await self._with_lang(self.startCommand, update, context)
    ...
    def BookPager(self, books, page_num, query):
        ...
        for row in books[start:finish + 1]:
            p = {'doubles': 0, 'lang_code': row.lang_code, 'filename': row.filename, 'path': row.path,
                 ...
                 'docdate': row.docdate, 'format': row.format, 'title': row.title,
        ...
        if summary_DOUBLES_HIDE:
            ...
            while ((finish + 1) < books_count) and double_flag:
                ...
                if books[finish].title.upper() == prev_title.upper() and \
                        {a['id'] for a in books[finish].authors.values()} == prev_authors_set:
                    items[-1]['doubles'] += 1
                else:
                    double_flag = False
    ...
    async def getBooks(self, update, context):
        ...
        response_data = self.BookPager(books, 1, query)
        await update.message.reply_text(
            response_data['message'], parse_mode='HTML', reply_markup=response_data['buttons']
        )

    async def _getBooks(self, update, context):
        return await self._with_lang(self._with_auth, self.getBooks, update, context)
    ...
    async def getBooksPage(self, update, context):
        ...
        books = self.BookFilter(query)
        response_data = self.BookPager(books, page_num, query)
        await callback_query.edit_message_text(
            text=response_data['message'], parse_mode='HTML', reply_markup=response_data['buttons']
        )
    ...
    async def downloadBooks(self, update, context):
        ...
        await update.message.reply_text(response, parse_mode='HTML', reply_markup=markup)
    ...
    async def getBookFile(self, update, context):
        ...
        if document:
            await context.bot.send_document(
                chat_id=callback_query.message.chat_id, document=document, filename=filename
            )
            document.close()
    ...
    def start(self):
        ...
        try:
            ...
            application.add_handler(CommandHandler('start', self._startCommand))
            application.add_handler(
                MessageHandler(filters.TEXT & ~filters.COMMAND, self._getBooks)
            )
            application.add_handler(
