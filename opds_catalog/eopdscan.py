...
class opdsScanner:
    ...
    def log_options(self):
        self.logger.info(' ***** Starting eopds-scan...')
        self.logger.debug('OPTIONS SET')
        if config.EOPDS_ROOT_LIB!=None:       self.logger.debug('root_lib = %s'%config.EOPDS_ROOT_LIB)
        if config.EOPDS_FB2TOEPUB!=None: self.logger.debug('fb2toepub = %s'%config.EOPDS_FB2TOEPUB)
        if config.EOPDS_FB2TOMOBI!=None: self.logger.debug('fb2tomobi = %s'%config.EOPDS_FB2TOMOBI)
        if config.EOPDS_TEMP_DIR!=None:       self.logger.debug('temp_dir = %s'%config.EOPDS_TEMP_DIR)
        if config.EOPDS_FB2SAX != None:       self.logger.info('FB2SAX = %s'%config.EOPDS_FB2SAX)
    ...
    def inpskip_callback(self, inpx, inp_file, inp_size):
        ...
        self.rel_path=os.path.relpath(os.path.join(inpx,inp_file),config.EOPDS_ROOT_LIB)
        
        if config.EOPDS_INPX_SKIP_UNCHANGED and opdsdb.inp_skip(self.rel_path,inp_size):
    ...
    def processinpx(self,name,full_path,file):
        rel_file=os.path.relpath(file,config.EOPDS_ROOT_LIB)
        inpx_size = os.path.getsize(file)
        if config.EOPDS_INPX_SKIP_UNCHANGED and opdsdb.inpx_skip(rel_file,inpx_size):
            ...
            inpx.INPX_TEST_ZIP = config.EOPDS_INPX_TEST_ZIP  
            inpx.INPX_TEST_FILES = config.EOPDS_INPX_TEST_FILES 
            inpx.parse()
    ...
    def processfile(self,name,full_path,file,cat,archive=0,file_size=0):
        (n, e) = os.path.splitext(name)
        if e.lower() in config.EOPDS_BOOK_EXTENSIONS.split():
            rel_path=os.path.relpath(full_path,config.EOPDS_ROOT_LIB)
