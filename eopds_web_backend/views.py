...
from eopds_web_backend.settings import HALF_PAGES_LINKS

def eopds_login(function=None, redirect_field_name=REDIRECT_FIELD_NAME, url=None):
    actual_decorator = user_passes_test(
        lambda u: (u.is_authenticated if config.EOPDS_AUTH else True),
...
def eopds_processor(request):
    args={}
    args['app_title']=settings.TITLE
    args['eopds_auth']=config.EOPDS_AUTH
    args['eopds_version']=settings.VERSION
    args['alphabet'] = config.EOPDS_ALPHABET_MENU
    args['splititems'] = config.EOPDS_SPLITITEMS
...
def SearchBooksView(request):
    ...
    if request.GET:
        ...
        #    return render_to_response('eopds_error.html', args)
        ...
        op = OPDS_Paginator(books_count, 0, page_num, config.EOPDS_MAXITEMS, HALF_PAGES_LINKS)
        items = []
        ...
        summary_DOUBLES_HIDE =  config.EOPDS_DOUBLES_HIDE and (searchtype != 'd')
        ...
        for row in books[start:finish+1]:
            p = {'doubles':0, 'lang_code': row.lang_code, 'filename': row.filename, 'path': row.path, \
                  ...
                  'readtime':row.bookshelf_set.filter(user=request.user).values('readtime') if config.EOPDS_AUTH else None
                 }
        ...
        args['cache_t']=config.EOPDS_CACHE_TIME
        
    return render(request,'eopds_books.html', args)
...
def SearchSeriesView(request):
    ...
    if request.GET:
        ...
        op = OPDS_Paginator(series_count, 0, page_num, config.EOPDS_MAXITEMS, HALF_PAGES_LINKS)        
        items = []
    ...
    return render(request,'eopds_series.html', args)
...
def SearchAuthorsView(request):
    ...
    if request.GET:
        ...
        op = OPDS_Paginator(authors_count, 0, page_num, config.EOPDS_MAXITEMS, HALF_PAGES_LINKS)        
        items = []
    ...
    return render(request,'eopds_authors.html', args)
...
def CatalogsView(request):   
    ...
    op = OPDS_Paginator(catalogs_count, books_count, page_num, config.EOPDS_MAXITEMS, HALF_PAGES_LINKS)
    items = []
    ...
    return render(request,'eopds_catalogs.html', args)  
...
def BooksView(request):   
    ...
    return render(request,'eopds_selectbook.html', args)      
...
def AuthorsView(request):   
    ...
    return render(request,'eopds_selectauthor.html', args)
...
def SeriesView(request):   
    ...
    return render(request,'eopds_selectseries.html', args)
...
def GenresView(request):   
    ...
    return render(request,'eopds_selectgenres.html', args)
...
def hello(request):
    ...
    return render(request, 'eopds_hello.html', args)
...
def LoginView(request):
    ...
    try:
        ...
        return render(request, 'eopds_login.html', args)
...
def handler403(request,args):
    response = render(request, 'eopds_login.html', args)
...
def Fb2ReaderView(request, book_id, page=1):
    ...
    return render(request, 'eopds_fb2reader.html', args)
